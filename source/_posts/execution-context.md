...
变量v明明在调用之前就已经定义过，可打印结果还是undefined，为什么？
...
简单的解释一下以上代码：首先定义了一个全局变量v，并对其赋值，这里属于全局作用域；然后定义了一个匿名函数，定义了一个局部变量v,存在于该函数自身作用域；最后自执行该匿名函数，由于函数内部也定义了变量v，所以不会去全局作用域中去取值，又因为内部变量v是用关键字“var”声明的，所以该声明会被提升到函数内代码的头部，以上函数执行效果同下：
...
以上可能还不能够说清楚“变量提升”这个概念，相信看完后面介绍的执行上下文后，会明白的。
...
以上比较抽象地描述了“执行上下文”，其实可以将其看作一个对象。每个执行上下文对象有三个必需属性（变量对象、作用域链以及this）：
